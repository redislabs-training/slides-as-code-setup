...
"init")
    ...
    if docker run --rm -d -v $PWD:/src docker.pkg.github.com/redislabs-training/slides-as-code/rl-slides-as-code:1.0.5 init; then
        printf "${HIGHLIGHT1}You presentation ${HIGHLIGHT2}presentation.md${HIGHLIGHT1} was initialised successfully. Serve it by running ./rls.sh serve and Have fun!${NC}\n\n"
    else
...
"serve")
    ...
    (sleep 2 && open http://localhost:${PORT_NUMBER}/presentation.html) & docker run --rm --init -p ${PORT_NUMBER}:${PORT_NUMBER} -v $PWD:/src docker.pkg.github.com/redislabs-training/slides-as-code/rl-slides-as-code:1.0.5 serve -s -p ${PORT_NUMBER}
    ;;
"export")
    if [ "$INSTALLATION_TYPE" = "npm" ]; then
    rls export --l false
    ...
    if docker run --rm -d -v $PWD:/src docker.pkg.github.com/redislabs-training/slides-as-code/rl-slides-as-code:1.0.5 export --l false; then
...
"pdf")
    ...
    docker run --rm -d -v $PWD:/src docker.pkg.github.com/redislabs-training/slides-as-code/rl-slides-as-code:1.0.5 pdf
    # printf "TODO"
    ;;
*) 
    printf "${HIGHLIGHT2}"
